...
namespace Microsoft.BotBuilderSamples
{
    public class UserProfileDialog : ComponentDialog
    {
        ...
        private static async Task<DialogTurnResult> Choosedepartmant(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            return await stepContext.PromptAsync(nameof(ChoicePrompt),
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Lütfen bir departman seçiniz"),
                    Choices = ChoiceFactory.ToChoices(new List<string> { "A", "B", "C" }),
                }, cancellationToken);
        }

        private static async Task<DialogTurnResult> InfoStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            stepContext.Values["transport"] = ((FoundChoice)stepContext.Result).Value;

            return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Not olarak kaydetmek istediğiniz bir şey var mı?") }, cancellationToken);
        }
        ...
        private async Task<DialogTurnResult> NameConfirmStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            await stepContext.Context.SendActivityAsync(MessageFactory.Text($"Notunuzu kaydediyorum: ' {stepContext.Result} '."), cancellationToken);
...
*/
        ...
        private static async Task<DialogTurnResult> PictureStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                var promptOptions = new PromptOptions
                {
                    Prompt = MessageFactory.Text("Fotoğraf ekleyin (ya da herhangi bir şey yazarak bu adımı geçin)."),
                    RetryPrompt = MessageFactory.Text("Fotoğrafınız jpeg/png formatında olmalı."),
                };
        ...
        private async Task<DialogTurnResult> ConfirmStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            // WaterfallStep always finishes with the end of the Waterfall or with another dialog; here it is a Prompt Dialog.
            return await stepContext.PromptAsync(nameof(ConfirmPrompt), new PromptOptions { Prompt = MessageFactory.Text("Fotoğrafını onaylıyor musun?") }, cancellationToken);
        }
        */
